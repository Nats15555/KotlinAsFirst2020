...
fun waveHtml(string: String): String {
    var splitString = Regex("""[^~]""").split(string).toMutableList()
    var splitStringNot = Regex("""(?:\~)+""").split(string).toMutableList()
    splitString.removeAll { it == "" }
    splitStringNot.removeAll { it == "" }
    var str = ""
    var buff = true
    var i = splitStringNot.size - 1
    if (splitString.size - splitStringNot.size < 0) {
        str = splitStringNot[0]
        splitStringNot.removeAt(0)
    } else {
        str = ""
    }
    var m = 0
...
fun equalsInStarHtml(string: String): String {
    ...
    while (i > 0) {
        when {
            splitStringStar[m].length == 1 && buffOneStar == false -> {
                str += "</i>" + splitStringNotStar[m]
                buffOneStar = true
            }
            splitStringStar[m].length == 1 && buffOneStar == true -> {
                str += "<i>" + splitStringNotStar[m]
                buffOneStar = false
            }
            splitStringStar[m].length == 2 && buffTwoStar == false -> {
                str += "</b>" + splitStringNotStar[m]
                buffTwoStar = true
            }
            splitStringStar[m].length == 2 && buffTwoStar == true -> {
                str += "<b>" + splitStringNotStar[m]
                buffTwoStar = false
            }
            splitStringStar[m].length == 3 && (buffThreeStar == false || (buffOneStar == false && buffTwoStar == false)) -> {
                if (splitStringStar[m - 1].length == 1) {
            ...
            splitStringStar[m].length >= 3 && buffThreeStar == true -> {
                if (m == 0) {
                    str += "<b><i>" + splitStringNotStar[m]
                    buffTwoStar = false
                    buffOneStar = false
                    ...
                    when {
                        splitStringStar[m].length - 3 == 3 && buffOneStar == false && buffTwoStar == false -> {
                            if (splitStringStar[m - 1].length == 1) {
